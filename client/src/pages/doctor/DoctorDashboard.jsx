...
import { Calendar, Clock, Users, TrendingUp, Stethoscope } from 'lucide-react';
...
const DoctorDashboard = () => {
  ...
  const statCards = [
    { icon: Calendar, label: "Today's Appointments", value: stats?.todayAppointments || 0, color: 'from-blue-500 to-blue-600', bgGlow: 'bg-blue-500/5' },
    { icon: Users, label: 'Total Patients', value: stats?.totalPatients || 0, color: 'from-emerald-500 to-emerald-600', bgGlow: 'bg-emerald-500/5' },
    { icon: Clock, label: 'Pending Reviews', value: stats?.appointmentsByStatus?.find(s => s._id === 'pending')?.count || 0, color: 'from-amber-500 to-amber-600', bgGlow: 'bg-amber-500/5' },
    { icon: TrendingUp, label: 'Completed Today', value: stats?.appointmentsByStatus?.find(s => s._id === 'completed')?.count || 0, color: 'from-violet-500 to-violet-600', bgGlow: 'bg-violet-500/5' },
  ];
      ...
      <div className="relative bg-gradient-to-br from-medical-600 to-medical-800 rounded-2xl p-6 md:p-8 overflow-hidden">
        ...
        <div className="relative z-10 flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-medical-200 mt-1">Welcome back, Doctor. Here's your day ahead.</p>
          </div>
        </div>
      </div>
      ...
      <div className="relative bg-white/80 dark:bg-dark-800/80 backdrop-blur-xl rounded-2xl border border-gray-100/50 dark:border-dark-700/50 shadow-lg dark:shadow-gray-900/30 overflow-hidden">
        ...
        <div className="p-6">
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="flex items-center gap-2 text-sm text-medical-600 dark:text-medical-400">
